...
const CheckListInterventionVidange = ({ device, onSubmit, onCancel, showNavbar }) => {
  const [formData, setFormData] = useState({
    ...
    date_prochain_vidange: device.date_prochain_vidange || '',
    ...
    type_organe: device.type_organe || '',
    gamme: device.gamme || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Soumission de l'opération de vidange
      await axios.post(
        `${process.env.REACT_APP_API_URL}/operationvidange`,
        formData,
        { headers: { Authorization: `Bearer ${token}` } }
      );

      // Mise à jour de la date de prochain vidange
      await axios.put(
        `${process.env.REACT_APP_API_URL}/devices/${device.id}/date-prochain-vidange`,
  ...
  <>
  <h3>Rapport de visite vidange</h3>
        ...
        <div className="ContainrColumns">
           ...
           <div className="col">
                                <p>
                                  ...
                                  <span
                                    style={{
                                      color: formData.date_prochain_vidange && new Date(formData.date_prochain_vidange) <= new Date() ? 'red' : 'black'
                                    }}>
                                    {formData.date_prochain_vidange ? new Date(formData.date_prochain_vidange).toLocaleDateString() : 'NA'}
                                  </span>
                                </p>
           ...
           <div className="col">
                                <p><strong>Quantité huile (L) :</strong> {formData.huile_quantity || 'NA'} <strong> - Type organe :</strong> {formData.type_organe || 'NA'}</p>
...
export default CheckListInterventionVidange;
