...
import CheckListIntervention from './CheckListIntervention';
...
const PlanningList = () => {
  ...
  const [endDate, setEndDate] = useState(today); // End date
  // États pour les champs de recherche
  const [searchTermNumero, setSearchTermNumero] = useState('');
  const [searchTermDesignation, setSearchTermDesignation] = useState('');
  const [filteredDevices, setFilteredDevices] = useState([]);

  useEffect(() => {
        if (endDate) {
          fetchDevices();
          fetchOperations(); // Ensure fetchOperations is called after the dates change
        }
        checkUserType();
    }, [endDate]); // Trigger when endDate changes
  ...
  // Utilisation de l'effet pour filtrer en fonction des deux critères de recherche
  useEffect(() => {
      const filtered = devices.filter((device) =>
        device.device_name.toLowerCase().includes(searchTermDesignation.toLowerCase()) &&
        device.numero_inventaire.toLowerCase().includes(searchTermNumero.toLowerCase())
      );
      setFilteredDevices(filtered);
  }, [searchTermNumero, searchTermDesignation, devices]);
      ...
      try {
        ...
        if (Array.isArray(response.data)) {
          const filteredDevices = response.data.filter(device => {
                      ...
                      return prochainGraissageDate <= end;
          });
  ...
  // Fonction d'exportation en CSV
      const exportToCSV = () => {
        ...
        const rows = devices.map(device => [
          device.date_prochain_graissage
            ? new Date(device.date_prochain_graissage).toLocaleDateString('fr-FR')
            : 'NA',
      ...
      const exportToExcel = () => {
        ...
        const data = devices.map(device => ({
          'Date': device.date_prochain_graissage
            ? new Date(device.date_prochain_graissage).toLocaleDateString('fr-FR')
            : 'NA',
...
<div className="date-input-container">
              <label>
                Planning jusqu'au :
                <input class="date-field" type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} />
              </label>
    </div>
            ...
            <div className="recherche"><input
                                  ...
                                  value={searchTermNumero}
                                  onChange={(e) => setSearchTermNumero(e.target.value)}
                                /></div>
                                <div className="recherche"><input
                                  type="text"
                                  placeholder="Rechercher par désignation"
                                  value={searchTermDesignation}
                                  onChange={(e) => setSearchTermDesignation(e.target.value)}
                                /></div>
        ...
        <tbody>
          {filteredDevices.map((device) => (
            <tr key={device.id}>
              ...
              <td className="abc">{new Date(device.date_prochain_graissage).toLocaleDateString('fr-FR')}</td>
              ...
              <td className="actions-cell" className="abc">
                <button onClick={() => handleEdit(device)}>Intervention graissage</button>
